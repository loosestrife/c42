...
/**
 * Extracts structured JSON graph data showing Promise relationships and function call flows from the c42 Document IR.
 * @param {Object} document - The c42 Document IR.
 * @param {Object} options - Visualization tuning options.
 * @param {boolean} [options.separateClosures=true] - Whether to give closures their own distinct nodes.
 * @param {boolean} [options.separateCatchBlocks=false] - Whether to split catch blocks into separate context nodes.
 * @returns {Object} Structured graph JSON containing contexts, promises, and edges.
 */
function extractPromiseGraph(document, options = {}) {
  const {
    separateClosures = true,
  ...
  return {
    contexts: Array.from(contexts.entries()).map(([id, label]) => ({ id, label })),
    promises: Array.from(promises.entries()).map(([id, label]) => ({ id, label })),
    edges
  };
}

/**
 * Renders structured graph JSON into a DOT format string.
 * @param {Object} graphData - Structured graph JSON containing contexts, promises, and edges.
 * @returns {string} DOT file content.
 */
function renderPromiseGraphDot(graphData) {
  let dot = 'digraph PromiseStateMachine {\n';
  dot += '  layout=neato;\n';
  dot += '  overlap=false;\n';
  dot += '  splines=true;\n';
  ...
  graphData.contexts.forEach(ctx => {
    const safeLabel = ctx.label.replace(/"/g, '\\"');
    dot += `  "${ctx.id}" [label="${safeLabel}", shape=box, style=rounded];\n`;
  });
  dot += '\n';

  graphData.promises.forEach(prom => {
    const safeLabel = prom.label.replace(/"/g, '\\"');
    dot += `  "${prom.id}" [label="${safeLabel}", shape=ellipse, style=filled, fillcolor=lightcyan];\n`;
  });
  dot += '\n';

  const uniqueEdges = [...new Set(graphData.edges.map(e => {
    const escapedLabel = e.label.replace(/"/g, '\\"');
    let attrs = `label="${escapedLabel}", color="${e.color}", fontcolor="${e.fontcolor}"`;
    if (e.extra) {
      attrs += `, ${e.extra}`;
    }
    if (e.arrowhead) {
      attrs += `, arrowhead="${e.arrowhead}"`;
    }
    return `  "${e.source}" -> "${e.target}" [${attrs}];`;
  }))];

  uniqueEdges.forEach(edgeStr => {
    dot += edgeStr + '\n';
  });
...
module.exports = { extractPromiseGraph, renderPromiseGraphDot };